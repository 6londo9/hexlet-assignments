...
class AppTest {
    ...
    // BEGIN
    @Test
    void testNotEmptyText() {
        CharSequence text = new ReversedSequence("testytest");

        String expectedReverse = "tsetytset";
        char expectedChar = 's';
        int expectedLength = 9;
        String expectedSubsequence = "set";

        String actualReverse = text.toString();
        char actualChar = text.charAt(1);
        int actualLength = text.length();
        String actualSubsequence = text.subSequence(1, 4).toString();

        assertThat(actualReverse).isEqualTo(expectedReverse);
        assertThat(actualChar).isEqualTo(expectedChar);
        assertThat(actualLength).isEqualTo(expectedLength);
        assertThat(actualSubsequence).isEqualTo(expectedSubsequence);
    }
    // END
}
