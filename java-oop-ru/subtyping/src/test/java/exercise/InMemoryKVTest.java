...
class InMemoryKVTest {
    ...
    void testIfFileKVImmutable() {
        ...
        KeyValueStorage kv = new FileKV("./src/test/resources/testKV", initial);

        initial.put("key2", "value2");
        assertThat(kv.toMap()).isEqualTo(clonedInitial);
    ...
    @Test
    void fileKVTest() {
        KeyValueStorage storage = new FileKV("./src/test/resources/testKV", Map.of("key", "10"));
        assertThat(storage.get("key2", "default")).isEqualTo("default");
        assertThat(storage.get("key", "default")).isEqualTo("10");
